...
- **workspace bootstrap** → `uvx dlthub-start@latest my-workspace` — when `dlthub ai status` reports missing prerequisites (no workspace, MCP not connected, missing dependencies). This is the authoritative bootstrap command for the dltHub AI workbench. **Do not run it yourself** — it is interactive and does not work when an agent runs it. **Ask the user to run it**: in Claude Code / Codex via `!` mode (`! uvx dlthub-start@latest my-workspace`), or in Cursor's integrated terminal. As a last-resort fallback only if the user declines, you may run `uv add "dlt[hub]"` (installs dlt only, no workspace scaffold). Re-check `dlthub ai status` once they confirm completion.
