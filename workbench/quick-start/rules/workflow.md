...
- **workspace bootstrap** — when `dlthub ai status` reports missing prerequisites (no workspace, MCP not connected, missing dependencies), pick the path that matches the user's situation:
  - **Existing project (almost everyone already in a project):** drive the `bootstrap` / `/bootstrap:init-workspace` flow in place — `uv add "dlt[hub]"` (installs `dlt[hub]`, not plain `dlt`), `uv run dlthub init`, `uv run dlthub ai init`. Do NOT use `uvx dlthub-start` here; it scaffolds a separate playground workspace.
  - **No working project yet (fresh onboarding):** ask the user to run `uvx dlthub-start@latest` (a playground scaffold, not for production workflows). **Do not run it yourself** — it is interactive. In Claude Code / Codex the user can use `!` mode (`! uvx dlthub-start@latest`); in Cursor, the integrated terminal.
  - Re-check `dlthub ai status` once they confirm completion.
