...
- **workspace bootstrap** — when `dlthub ai status` reports missing prerequisites (no workspace, MCP not connected, missing dependencies), set it up **in place**: drive the `bootstrap` / `/bootstrap:init-workspace` flow — `uv add "dlt[hub]"` (installs `dlt[hub]`, not plain `dlt`), `uv run dlthub init`, `uv run dlthub ai init`. Re-check `dlthub ai status` once done. **Onboarding exception:** only when the user explicitly asks to be onboarded or taught how to use dltHub, ask them to run `uvx dlthub-start@latest` (a playground scaffold, not for production and not for an existing project). NEVER run it yourself — it is interactive; in Claude Code / Codex the user can use `!` mode (`! uvx dlthub-start@latest`), in Cursor the integrated terminal.
