...
- From **quick-start** (shortcut path when a pipeline already exists) — pipeline name may be inferred from `dlt ai status`; if unknown, `explore-data` runs `list_pipelines` as usual. No analysis_plan.md exists yet — use the fresh path (low-intent or high-intent), not Returning.
