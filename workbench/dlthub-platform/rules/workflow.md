...
5. **Evaluate the inspector** (`job-inspector-eval`) — background agent: runs after every inspector run and grades the diagnosis against the inspector's own instructions, `TRUE`, `FALSE` or `N/A` per instruction with a reasoning. It ships no default model, so the judge model is a deployment decision: set `AGENT__MODEL` as a workspace variable, which takes a `provider:model` id on any provider. The short aliases (`sonnet`, `gpt-mini`, `gemini`) exist on Anthropic, OpenAI and Google, so a job pinned to `model="sonnet"` fails on an Azure-backed workspace. See the agent's `README.md` for the deployment snippet and the recommendation per provider
