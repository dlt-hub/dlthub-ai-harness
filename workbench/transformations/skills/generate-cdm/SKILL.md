...
**Requires** `.schema/<cdm-name>/ontology_model.py` from `create-ontology`.
If missing, run `create-ontology` first.

Read `ontology.cdm_name` from `.schema/<cdm-name>/ontology_model.py` to determine `<cdm-name>` — all files in this skill are under that folder.
...
Read `.schema/<cdm-name>/ontology_model.py`. For each entity, apply Kimball classification:
...
Check for aliases that only become visible at the dimensional modeling stage — e.g. two ontology entities that would produce identical dimension tables. Do **not** re-open concept collapses already confirmed in `ontology_model.py`; those are settled.
