...
description: "Testing/educational path for trying dlthub, onboarding, or a quick demo — build and run a minimal single-endpoint REST API pipeline locally against DuckDB with a row limit, to see data flow fast. Use when the user names a REST API source and just wants to test it / kick the tires / see a demo. Educational examples only — NOT a production-grade pipeline (auth, incremental, multiple endpoints); for that use the `rest-api-pipeline` toolkit (`find-source`)."
