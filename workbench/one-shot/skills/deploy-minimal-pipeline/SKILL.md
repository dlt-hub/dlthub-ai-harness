...
description: Test-deploy the minimal REST API pipeline to dltHub Platform to see it run on the cloud — an educational first-run validation, NOT a production-grade deployment. Use when the user has a working local pipeline from create-minimal-pipeline and wants to try running it on the cloud. Assumes workspace setup, login, and deployment file are already in place — destination credentials are configured here.
---
